...
describe('cli.ts', () => {
  ...
  it('#cli', async () => {
    ...
    const descriptionStkArgs = commanderModuleTesting
      .getResult('description')
      .getStackedArgs();
    const commandStkArgs = commanderModuleTesting
      .getResult('command')
      .getStackedArgs();
    const optionStkArgs = commanderModuleTesting
      .getResult('option')
      .getStackedArgs();
    const buildCommand = commanderModuleTesting
      .getResult('action')
      .getArgInStack(1, 1);
    const helpCommand = commanderModuleTesting
      .getResult('action')
      .getArgInStack(2, 1);
    const anyCommand = commanderModuleTesting
      .getResult('action')
      .getArgInStack(3, 1);
    // commander data
    ...
    const errorStub = sinon
      .stub(console, 'error')
      .callsFake((value: any) => value);
    const anyCommandResult = await anyCommand('xxx');
    expect(anyCommandResult).equal("\u001b[31mUnknown command 'xxx'\u001b[39m");
    errorStub.restore();
  });
});
