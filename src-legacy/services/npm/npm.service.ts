...
import { NPMPackageDotJson } from './npm.type';
import { SHEETBASE_MODULE_FILE_NAME } from '../code/code.config';

export async function packageJson(path = '.'): Promise<NPMPackageDotJson> {
...
export async function getSheetbaseChildDependencies(parents: string[]): Promise<string[]> {
    ...
    for (let i = 0; i < parents.length; i++) {
        ...
        } = await packageJson(`./node_modules/${parent}`);
        resultDependencies = resultDependencies.concat([
